...
function SalesHistoryPage() {
  ...
  const cancelSale = async (sale: Sale) => {
    setDialogConfig({
      ...
      message: "Digite a senha de gerente para autorizar este cancelamento:",
      onConfirm: async (password) => {
        if (!password) {
          toast.error("Senha necessária para cancelar.");
          return;
        }

        const isAuthorized = await validateManagerPassword(password);
        if (!isAuthorized) {
          toast.error("Senha incorreta.");
          return;
        }

        const { error } = await supabase.rpc("cancel_sale_with_stock", {
          p_sale_id: sale.id,
        });
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium">
            Relatório detalhado de transações realizadas.
          </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          <button
            onClick={exportPDF}
            className="flex items-center gap-2 bg-emerald-600 text-white px-5 py-2.5 rounded-xl font-bold hover:bg-emerald-700 transition-all">
                  ...
                  sales?.map((sale) => (
                    <tr
                        ...
                        <div className="flex flex-col gap-0.5">
                          <span
                            ...
                            {sale.sale_items
                              .map(
                                (i) =>
                                  `${i.quantity}x ${i.products?.name || "n/a"}`,
                              )
                              .join(", ")}
                          </span>
                      ...
                      <td className="p-4 sm:p-6 text-right">
                        ...
                        {sale.status === "canceled" && (
                          <span className="text-[10px] font-black text-red-400 uppercase tracking-widest bg-red-50 dark:bg-red-900/20 px-2 py-1 rounded-lg">
                            Estornado
                          </span>
                        )}
                      </td>
