...
function SalesPage() {
  ...
  const finalizeSale = async (method: string, amount: number) => {
    ...
    try {
      ...
      const existing = JSON.parse(localStorage.getItem('pending_sales') || '[]');
      localStorage.setItem('pending_sales', JSON.stringify([...existing, pendingData]));
    ...
    <div className="h-full flex flex-col gap-6 p-2 lg:p-4">
      ...
      <div className="flex flex-col lg:flex-row gap-6 h-[calc(100vh-220px)]">
        ...
        <div className="flex-1 flex flex-col gap-4 min-w-0 h-full">
              ...
              {searchResults.map(p => (
                ...
                >
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs text-slate-500 uppercase tracking-wider">Estoque: {p.stock_quantity} {p.unit}</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="font-mono font-bold text-indigo-600 dark:text-indigo-400">R$ {p.sale_price.toFixed(2)}</span>
          ...
          <div className="flex-1 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden flex flex-col">
            <div className="p-4 border-b border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 flex justify-between items-center">
              <span className="text-sm font-bold text-slate-500 uppercase tracking-widest">Itens no Carrinho</span>
              <span className="bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 px-2.5 py-0.5 rounded-full text-xs font-bold">{cart.length} itens</span>
            </div>
        ...
        <div className="w-full lg:w-96 flex flex-col gap-4">
          <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-xl shadow-slate-200/50 dark:shadow-none flex flex-col gap-6">
            <div>
              <div className="text-sm text-slate-400 uppercase font-black tracking-widest mb-1">Total da Venda</div>
            ...
            <div className="pt-4 border-t border-slate-100 dark:border-slate-700 grid grid-cols-2 gap-2 text-[10px] uppercase font-bold text-slate-400 tracking-tighter">
              <div className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-indigo-400" /> Atalhos Ativos</div>
              <div className="flex items-center gap-1.5"><div className="w-1.5 h-1.5 rounded-full bg-emerald-400" /> Sincronização OK</div>
            </div>
          </div>
          ...
          <div className="hidden lg:block bg-indigo-50 dark:bg-indigo-900/10 border border-indigo-100 dark:border-indigo-900/30 p-4 rounded-2xl">
            <h4 className="text-xs font-black text-indigo-600 dark:text-indigo-400 uppercase mb-2">Dica do Sistema</h4>
            <p className="text-xs text-indigo-800/70 dark:text-indigo-300/60 leading-relaxed">
              Use <strong>F1</strong> para buscar rápido e <strong>ESC</strong> para limpar o carrinho se desistir da venda.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tight">Receber Dinheiro</h3>
              <button onClick={() => setShowCashModal(false)} className="text-slate-400 hover:text-slate-600"><X /></button>
            </div>
            
            <div className="space-y-4">
              <div>
                <label className="text-xs font-bold text-slate-400 uppercase mb-1 block tracking-wider">Valor Recebido</label>
                <input 
                  type="number" 
                  autoFocus
              ...
              <div className="bg-slate-50 dark:bg-slate-900/50 p-6 rounded-2xl border border-slate-100 dark:border-slate-700">
                ...
                <div className="flex justify-between items-center pt-2 border-t border-slate-200 dark:border-slate-700">
                  <span className="font-bold text-slate-700 dark:text-slate-300 uppercase text-xs">Troco a devolver</span>
                  <span className={`text-3xl font-black ${parseFloat(receivedCash) >= total ? 'text-emerald-600' : 'text-slate-400'}`}>
                    R$ {Math.max(0, parseFloat(receivedCash) - total || 0).toFixed(2)}
                  </span>
                </div>
              </div>
      ...
      {showCardModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-slate-800 p-8 rounded-3xl w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-black text-slate-800 dark:text-white uppercase tracking-tight text-center w-full">Pagamento Cartão</h3>
            </div>
            ...
            <div className="space-y-6">
              {cardType === 'credit' && (
                <div className="animate-in fade-in slide-in-from-top-2 duration-200">
                  <label className="text-xs font-bold text-slate-400 uppercase mb-2 block tracking-wider text-center">Selecionar Parcelas</label>
              ...
              <div className="bg-indigo-50 dark:bg-indigo-900/10 p-6 rounded-3xl border border-indigo-100 dark:border-indigo-900/30 text-center">
                <div className="text-xs font-bold text-indigo-400 uppercase mb-1">Valor na Maquininha</div>
                <div className="text-4xl font-black text-indigo-600 dark:text-indigo-400 tracking-tighter">R$ {total.toFixed(2)}</div>
                <div className="text-[10px] font-bold text-indigo-500/60 uppercase mt-2 tracking-widest">
                  {cardType === 'debit' ? 'MODALIDADE DÉBITO' : `MODALIDADE CRÉDITO ${installments}X`}
                </div>
              </div>
